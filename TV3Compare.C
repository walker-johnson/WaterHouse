...
void TV3Compare()
{
  ...
  TH1F *xh[3];
  xh[0] = new TH1F("1D flux", "D_{#gamma} 1.5m from Shielding Surface", 40, -2, 2);
  xh[1] = new TH1F("1D flux 2", "Flux Comparison 2", 40, -2, 2);
  xh[2] = new TH1F("1D flux 3", "Flux Comparison 3", 40, -2, 2);

  TH1F *xhn[3];
  xhn[0] = new TH1F("1D flux  ", "D_{n} 1.5m from Shielding Surface", 40, -2, 2);
  ...
  for(int i = 0 ; i < 3; i++ ){
    ...
    TTreeReader *testReader = new TTreeReader("gTV3", f);
    TTreeReader *testReaderN = new TTreeReader("nTV3", f);
    ...
    while(testReader->Next()){
      if(*z >= planeLoc[2][4] && *y-yCenter >= -0.3 && *y-yCenter <= -0.2){
	xh[i]->Fill(*x);  
      }
    }

    while(testReaderN->Next()){
      if(*zN >= planeLoc[2][4] && *yN-yCenter >= -0.3 && *yN-yCenter <= -0.2){
	xhn[i]->Fill(*xN);
      }
    }
  ...
  TH1F *x1 = new TH1F("Dose Rate", "D_{net} 1.5m Shielding Surface", 40, -2, 2);
  x1->Add(xh[0], xhn[0]);
