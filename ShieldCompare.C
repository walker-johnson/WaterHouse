...
void ShieldCompare()
{
  ...
  for(int i = 0 ; i < 3; i++ ){
    ...
    TTreeReader *testReader = new TTreeReader("gShield", f);
    TTreeReader *testReaderN = new TTreeReader("nShield", f);
    ...
    while(testReader->Next()){
      if(*z >= planeLoc[0][4] - 0.51 && *y-yCenter >= -0.3 && *y-yCenter <= -0.2){
	xh[i]->Fill(*x);  
      }
    }

    while(testReaderN->Next()){
      if(*zN >= planeLoc[0][4] - 0.51 && *yN-yCenter >= -0.3 && *yN-yCenter <= -0.2){
	xhn[i]->Fill(*xN);
      }
    }
