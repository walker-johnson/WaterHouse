...
void TV2Compare()
{
  ...
  TH1F *xh[3];
  xh[0] = new TH1F("1D flux", "D_{#gamma} 1m from Shielding Surface", 40, -2, 2);
  xh[1] = new TH1F("1D flux 2", "Flux Comparison 2", 40, -2, 2);
  xh[2] = new TH1F("1D flux 3", "Flux Comparison 3", 40, -2, 2);

  TH1F *xhn[3];
  xhn[0] = new TH1F("1D flux  ", "D_{n} 1m from Shielding Surface", 40, -2, 2);
  ...
  for(int i = 0 ; i < 3; i++ ){
    ...
    TTreeReader *testReader = new TTreeReader("gTV2", f);
    TTreeReader *testReaderN = new TTreeReader("nTV2", f);
    ...
    while(testReader->Next()){
      if(*z >= planeLoc[1][4] && *y-yCenter >= -0.3 && *y-yCenter <= -0.2){
	xh[i]->Fill(*x);  
      }
    }

    while(testReaderN->Next()){
      if(*zN >= planeLoc[1][4] && *yN-yCenter >= -0.3 && *yN-yCenter <= -0.2){
	xhn[i]->Fill(*xN);
      }
    }
